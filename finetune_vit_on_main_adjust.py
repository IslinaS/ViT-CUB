...
# DATA LOADERS
# --------------------------
# add in more data augmentations
...
# add in weight decay
optimizer = optim.Adam(filter(lambda p: p.requires_grad, vit.parameters()), lr=1e-4, weight_decay=1e-4)
criterion = nn.CrossEntropyLoss()

# --------------------------
# TRAINING LOOP
# --------------------------
freeze_epochs = 10  # <<< NEW: Number of epochs to freeze backbone
...
for epoch in range(num_epochs):
    ...
    if epoch == freeze_epochs:
        ...
        optimizer = optim.Adam(vit.parameters(), lr=1e-4, weight_decay=1e-4)  # <<< NEW: use smaller lr after unfreezing
...
tag = f'freeze{freeze_epochs}_epoch{num_epochs}'
torch.save(vit.state_dict(), f'vit_finetuned_pretrained_{tag}.pth')
print(f'Model saved as vit_finetuned_pretrained_{tag}.pth')
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title(f'Loss (freeze={freeze_epochs})')
plt.legend()
plt.subplot(1, 2, 2)
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title(f'Accuracy (freeze={freeze_epochs})')
plt.legend()
plt.tight_layout()
