...
# --------------------------
unfreeze_schedules = [0, 3, 5]  # freeze 0, 3, or 5 epochs
post_unfreeze_lrs = [1e-4, 1e-5]  # learning rates after unfreeze

# --------------------------
# MAIN GRID SEARCH LOOP
# --------------------------
for freeze_epochs in unfreeze_schedules:
    for post_unfreeze_lr in post_unfreeze_lrs:
        print(f'\n==== Running: freeze_epochs={freeze_epochs}, post_unfreeze_lr={post_unfreeze_lr} ====')
        ...
        for epoch in range(num_epochs):
            if epoch == freeze_epochs:
                ...
                optimizer = optim.Adam(vit.parameters(), lr=post_unfreeze_lr)
        ...
        # Save model + plot for each run
        tag = f'freeze{freeze_epochs}_lr{post_unfreeze_lr}'
        torch.save(vit.state_dict(), f'vit_finetuned_{tag}.pth')
        print(f'Model saved as vit_finetuned_{tag}.pth')
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title(f'Loss (freeze={freeze_epochs}, lr={post_unfreeze_lr})')
        plt.legend()
        plt.subplot(1, 2, 2)
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.title(f'Accuracy (freeze={freeze_epochs}, lr={post_unfreeze_lr})')
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'curves_{tag}.png')
        plt.close()
        print(f'Curves saved as curves_{tag}.png')
